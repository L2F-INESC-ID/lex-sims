    ...
    while pos <= last:
        ...
        if ch in vowels:
            ...
            if pos > first + 1 and st[pos - 2] not in vowels and st[pos - 1:pos + 2] == 'ACH' and \
                            st[pos + 2] not in ['I'] and (
                    st[pos + 2] not in ['E'] or st[pos - 2:pos + 4] in ['BACHER', 'MACHER']):
                nxt = ('K', 2)
            # special case 'CAESAR'
            ...
            # -ges-,-gep-,-gel-, -gie- at beginning
                                           ...
                                           or st[pos + 1:pos + 3] in ["ES", "EP", "EB", "EL", "EY", "IB", "IL", "IN",
                                                                      "IE", "EI", "ER"]):
                nxt = ('K', 'J', 2)
            # -ger-,  -gy-
...
def go_double_metaphone(lista):
    lista_demed = [dm(unicode(l)) for l in lista]
    lista_out = [x[0] for x in lista_demed]
    return lista_out


def stem_lowered(lista, stem=True):
    if stem:
...
def cria_dict(clusterfilename):
    global adict

    if os.path.isfile("cacheric/" + clusterfilename):
        print  "Cached cluster: cacheric/" + clusterfilename

        with open("cacheric/" + clusterfilename, 'rb') as f1:
            adict = pickle.load(f1)

        return

    print "building brown clusters..."
    f = open(clusterfilename, "r")  # cluster/brown-clusters-pt.txt
    for line in iter(f):
        linha = line.split("\t", 2)
        del linha[-1]
        adict[linha[1]] = linha[0]
    f.close()

    if not os.path.isfile("cacheric/" + clusterfilename):
        print  "caching clusters into: cacheric/" + clusterfilename
...
def clustering(token):
    global adict
    if adict.has_key(token):
        res = adict[token]
    else:
        res = "0"
    return res
...
def cluster(lista):
    return [clustering(x) for x in lista]
...
def character_trigram(lista):
    lista = lower_case(lista)
    lista_pre = pre_trigram(lista)
    out = pos_trigram(lista_pre)
    return out
